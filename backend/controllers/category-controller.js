    ...
    try {
      ...
      Category.findOne({ NAME: data.NAME }, (err, category_find) => {
        ...
        if (category_find) {
          ...
          _id_parent_new === '' ? (data.IS_PARENT = true) : (data.IS_PARENT = false)
    ...
    try {
      ...
      if (!isNameRepeated(data.NAME, _id)) {
        return res.send({
          status: 202,
          data: null,
          message: 'Ocurrió algún error o ya existe categoria con el mismo nombre.',
        })
      } else {
        if (
          //falta testear
          ...
          Category.findByIdAndUpdate(_id, data, { new: true }, (error, category_new) => {
